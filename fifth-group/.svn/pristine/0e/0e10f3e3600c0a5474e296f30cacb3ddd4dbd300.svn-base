<%@page language="java"  pageEncoding="utf-8"%>
<%@page language="java" contentType="text/html; charset=utf-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>杰仔专卖网-用户中心</title>
<meta name="Copyright" content="Douco Design." />
<link href="css/public.css" rel="stylesheet" type="text/css">
<script type="text/javascript" src="js/jquery.min.js"></script>
<script type="text/javascript" src="js/global.js"></script>
</head>
<body>
<div id="dcWrap">
<%@include file="Head.jsp" %>
<div id="dcMain">
	<div id="urHere"><b>杰仔专卖网-管理中心</b></div>   
		<div id="manager" class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
    		<h3>个人中心</h3>
    		<table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
    			<tr align="center" style="font-size: 20px;">
    				<td colspan="7">个人中心</td>
    			</tr>
     			<tr>
			    	<th width="30">编号</th>
			      	<th>用户名</th>
			      	<th>真实姓名</th>
			      	<th>E-mail地址</th>
			      	<th>性别</th>
			      	<th>年龄</th>
			      	<th>操作</th>
     			</tr>
          		<tr>
	      			<td align="center">${sessionScope.user.userid }</td>
	      			<td align="center">${sessionScope.user.username }</td>
	      			<td align="center">${sessionScope.user.realname }</td>
	      			<td align="center">${sessionScope.user.email }</td>
	      			<td align="center">${sessionScope.user.sex }</td>
	      			<td align="center">${sessionScope.user.age }</td>
	      			<td align="center"><a href="editmyself.jsp">编辑</a></td>
     			</tr>
         	</table>
        </div>
</div><!-- dcMain 结束 -->
<div class="clear"></div>
 	
<div id="dcFooter">
 	<div id="footer" align="center">
  		<div class="line"></div>
 			<p style="margin-top: 10px;">版权所有 © 2013-2015 漳州豆壳网络科技有限公司，并保留所有权利。</p>
 		</div>
	</div><!-- dcFooter 结束 -->
<div class="clear"></div> </div>

</body>
</html>